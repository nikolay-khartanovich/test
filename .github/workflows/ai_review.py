...
Найди только СЕРЬЕЗНЫЕ проблемы, которые могут сломать код или создать уязвимости:

**БЛОКИРУЮЩИЕ проблемы:**
- Критические баги (NPE, бесконечные циклы, неправильная логика)
- Уязвимости безопасности (SQL injection, XSS, CSRF)
- Утечки секретов (реальные пароли/токены в коде)
- Синтаксические ошибки
- Неопределенные переменные или функции
- Критические проблемы производительности

**НЕ анализируй мелочи:**
- Названия переменных в UPPER_CASE (это стандарт для констант)
- Хардкод версий в конфигах (3.11, etc - это нормально)
- Предложения разбить функции на более мелкие
- Хардкод текстов и сообщений
- Стиль кода и форматирование
- Архитектурные предложения (SOLID, DRY - если код работает)
- Мелкие проверки типов (если есть базовая валидация)

Для каждой СЕРЬЕЗНОЙ проблемы укажи файл и строку: `файл.js:строка`

Если серьезных проблем НЕТ - ответь точно: "НЕТ ПРОБЛЕМ"

Пример серьезных проблем:
```
**auth.js:15** - SQL инъекция: user_id напрямую в запрос без экранирования
**api.js:23** - Неопределенная переменная 'userToken' приведет к ReferenceError  
**config.js:8** - Реальный API ключ "sk-abc123" захардкожен в коде
```

ВАЖНО: Игнорируй мелкие недочеты! Блокируй только то, что может реально сломать систему.
"""
